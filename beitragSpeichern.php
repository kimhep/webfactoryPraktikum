...
if(!empty($_POST)) {
    ...
    // Tabelle ausfüllen
    $query = "INSERT INTO posts (user_id, title, teaser,  text, erstellungsDatum) VALUES (?, ?, ?, ?, NOW())";

    try {
        $stmt = $db->prepare($query);
        $result = $stmt->execute([$_SESSION["user_id"],$_POST["titel"], $_POST["teaser"], $_POST["text"]]);
